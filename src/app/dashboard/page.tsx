...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-indigo-950 to-slate-950 text-slate-100 flex flex-col relative overflow-hidden">
      ...
      <nav className="bg-slate-900/60 backdrop-blur-md border-b border-white/5 relative z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Logo size={28} className="cursor-pointer hover:scale-105 transition-all" onClick={() => router.push("/")} />
            <div className="flex flex-col">
              <span className="text-sm font-bold text-white tracking-tight">Sec+ Prep Dashboard</span>
              <span className="text-[9px] text-indigo-400 font-semibold tracking-widest uppercase mt-0.5">Performance Center</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <button
              onClick={() => router.push("/exam")}
              className="px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-750 text-white rounded-lg text-xs font-bold hover:scale-105 active:scale-95 transition-all shadow-md shadow-indigo-500/20"
            >
              Start Smart Exam
            </button>
            <button
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 w-full flex-1 relative z-10 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-900/40 backdrop-blur-xl p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all shadow-lg group">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider group-hover:text-indigo-400 transition-colors">Exams Taken</div>
            <div className="text-2xl md:text-3xl font-extrabold text-white mt-1.5">{stats.total}</div>
          </div>
          <div className="bg-slate-900/40 backdrop-blur-xl p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all shadow-lg group">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider group-hover:text-purple-400 transition-colors">Average Score</div>
            <div className="text-2xl md:text-3xl font-extrabold text-white mt-1.5">
              {stats.total > 0 ? `${stats.average}%` : "-"}
            </div>
          </div>
          <div className="bg-slate-900/40 backdrop-blur-xl p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all shadow-lg group">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider group-hover:text-emerald-400 transition-colors">Best Score</div>
            <div className="text-2xl md:text-3xl font-extrabold text-white mt-1.5">
              {stats.total > 0 ? `${stats.best}%` : "-"}
            </div>
          </div>
          <div className="bg-slate-900/40 backdrop-blur-xl p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all shadow-lg group">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider group-hover:text-orange-400 transition-colors">Questions Answered</div>
            <div className="text-2xl md:text-3xl font-extrabold text-white mt-1.5">
        ...
        {weakestDomains.length > 0 && (
          <div className="p-5 rounded-2xl border border-rose-500/20 bg-rose-950/10 backdrop-blur-lg flex flex-col md:flex-row md:items-center justify-between gap-4 pulse-glow-red">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <span className="text-lg">⚠️</span>
                <span className="font-bold text-rose-400 text-sm tracking-wide uppercase">Critical Focus Areas</span>
              </div>
            ...
            <button
              onClick={() => router.push("/exam")}
              className="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 active:scale-95 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-900/20 self-start md:self-center transition-all"
            >
              Practice Weak Areas
            </button>
          </div>
        ...
        {domainPerf.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/5 p-6 shadow-xl flex flex-col justify-between">
              <div>
                <h2 className="text-sm font-bold text-white tracking-wide uppercase mb-1">Performance Radar</h2>
                <p className="text-[10px] text-slate-500">Outer Ring = 100% | Dashed Zone = 75% Passing Threshold</p>
              </div>
            ...
            <div className="lg:col-span-3 bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/5 p-6 shadow-xl flex flex-col justify-between">
              <div>
                <h2 className="text-sm font-bold text-white tracking-wide uppercase mb-1">SY0-701 Exam Weightage</h2>
                <p className="text-[10px] text-slate-500 mb-4">Target answered volume to simulate actual exam balance</p>
              </div>
                  ...
                  return (
                    <div key={w.domain} className="space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <div className="flex items-center gap-2.5 text-slate-500 font-mono text-[10px] flex-shrink-0">
        ...
        {domainPerf.length > 0 && (
          <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/5 shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-white/5 bg-slate-900/20">
              <h2 className="text-sm font-bold text-white tracking-wide uppercase">Domain Mastery Metrics</h2>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {domainPerf.map((p) => {
                  ...
                  return (
                    <div
                      key={p.domain}
                      className={`p-4 rounded-xl border ${ringBorder} bg-slate-950/30 flex flex-col justify-between gap-3 hover:border-slate-800 transition-colors`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {p.streak >= 3 && (
                          <span className="text-[10px] text-amber-400 font-bold flex-shrink-0 animate-pulse">
                            🔥 {p.streak} streak
                          </span>
                        )}
                      </div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/5 shadow-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/5 bg-slate-900/20">
            <h2 className="text-sm font-bold text-white tracking-wide uppercase">Exam History Log</h2>
          </div>
          {attempts.length === 0 ? (
            <div className="p-12 text-center space-y-4">
              <div className="text-5xl animate-pulse">📝</div>
              <p className="text-slate-500 text-sm">No exam sessions logged yet.</p>
              <button
                onClick={() => router.push("/exam")}
                className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl text-xs font-bold hover:scale-105 active:scale-95 transition-all shadow-md shadow-indigo-500/20"
              >
            ...
            <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto">
              {attempts.map((a) => {
                ...
                return (
                  <div key={a.id} className="px-6 py-4 hover:bg-white/[0.02] transition-colors space-y-3">
                    ...
                    {a.domainResults && (
                      <div className="flex flex-wrap gap-1.5">
                        {a.domainResults.map((d) => {
                          ...
                          const scoreColor = pct >= 75 ? "text-emerald-400 bg-emerald-500/5 border-emerald-500/10" : pct >= 50 ? "text-amber-400 bg-amber-500/5 border-amber-500/10" : "text-rose-400 bg-rose-500/5 border-rose-500/10"
                          return (
                            <span
                              key={d.domain}
                              className={`text-[9px] font-medium px-2 py-0.5 rounded border ${scoreColor}`}
                            >
